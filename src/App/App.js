...
export const App = () => {
  ...
  const currentReliefCenter = useMemo(
    () => ({ reliefCenter, setreliefCenter }),
    [reliefCenter, setreliefCenter]
  );
  const users = useMemo(() => ({ currentUsers, setCurrentUsers }), [
    currentUsers,
    setCurrentUsers
  ]);
  const items = useMemo(() => ({ currentItems, setCurrentItems }), [
    currentItems,
    setCurrentItems
  ]);
          ...
          <UsersContext.Provider value={users}>
            <ItemsContext.Provider value={items}>
              ...
              <Route
                exact
                path="/check-in"
                render={() => <CheckInForm reliefCenter={reliefCenter} />}
              />
